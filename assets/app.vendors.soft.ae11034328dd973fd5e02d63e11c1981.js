 ...
 * @return  void
 */
HTMLDivElement.prototype.orderAlphabeticallyByText = function(selector) {
    var $div = $(this),
        $anchors = $div.find(selector).get();
    $anchors.sort(function(a, b) {
        return $(a).text().trim().toUpperCase().localeCompare(
            $(b).text().trim().toUpperCase()
        );
    })
    jQuery.each($anchors, function(index, element) {
        $div.append(element);
    });
};
 ...
 * @return  Boolean
 */
// String.prototype.validObject = function() {
 ...
 * validMethod
 * 
 * Helper method to check whether a method call against an object is valid to
 * be made, by checking whether the object and method themselves exist. For
 * example:
 * 
 * 'document.addEventListener'.validMethod() === true
 * 'document.addEventListeners'.validMethod() === false
 * 
 * @access  public
 * @return  Boolean
 */
// String.prototype.validMethod = function() {
//     var pieces = this.split('.'),
//         index,
//         reference = window;
//     for (index in pieces) {
...
Number.prototype.roundToLower = function(interval) {
    return Math.floor(this.toInt() / interval) * interval;
};
 ...
 * @return  Number
 */
Number.prototype.roundToLower50 = function() {
    return Math.floor(this.toInt() / 50) * 50;
};
 ...
 * if required, kill them (eg. when the app is frozen via Stencil.kill()).
 * 
 * @access  public
 ...
 * directly.
 * 
 * @note    Was using DataUtils for a wild for the argument validation, however
...
Function.prototype.proxy = function(context, args) {
    ...
    args.unshift(this);
    return jQuery.proxy.apply(jQuery, args);
};
